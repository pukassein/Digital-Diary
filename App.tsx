...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const fetchWithRetries = async (retries = 3, delay = 2500) => {
      setIsLoading(true);
      for (let i = 0; i < retries; i++) {
        ...
        // Retry only on timeout errors, which is the most likely culprit for intermittent failures
        // on free-tier services that may "sleep".
        if (error.message.toLowerCase().includes('timeout') && i < retries - 1) {
          console.log(`Database may be waking up. Retrying in ${delay / 1000} seconds...`);
          await new Promise(res => setTimeout(res, delay));
        } else {
          // On the last attempt or for a non-timeout error, show the message.
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-diary-bg">
        <div className="text-center">
            <svg className="animate-spin h-12 w-12 text-diary-accent mx-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="http://www.w3.org/2000/svg">
