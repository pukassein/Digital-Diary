...
const EntryList: React.FC<EntryListProps> = ({ entries, onSelectEntry, onCreateToday, onCreateForPastDate, onExport, userRole, onCloseDiary }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-diary-paper p-6 sm:p-8 relative overflow-hidden">
      {/* Background Icons */}
      <div className="absolute top-1/4 -left-12 text-diary-accent opacity-10 transform -rotate-12 z-0 pointer-events-none">
       ...
       <div className="absolute bottom-1/4 -right-16 text-diary-accent opacity-10 transform rotate-12 z-0 pointer-events-none">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1} stroke="currentColor" className="w-56 h-56">
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto textarea-scrollbar -mr-4 pr-4">
          {entries.length > 0 ? (
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {entries.map(entry => (
                <div
                  ...
                  className="aspect-square flex flex-col items-center justify-center p-2 bg-diary-bg rounded-lg cursor-pointer border-2 border-transparent hover:border-diary-accent hover:shadow-md transition-all duration-200"
                  role="button"
                  aria-label={`Open entry from ${fullFormatDate(entry.date)}`}
                >
                  <span className="text-2xl font-serif text-diary-dark">{formatCardDate(entry.date)}</span>
                </div>
              ))}
            </div>
          ) : (
            <div className="text-center py-10 text-diary-text font-sans flex flex-col items-center gap-4 h-full justify-center">
              ...
              {userRole === 'author' && <p>Click "New" to start writing a new page.</p>}
            </div>
          )}
        </div>
        <div className="mt-auto pt-6 border-t-2 border-dashed border-diary-accent/30 text-center">
          <p className="font-serif text-xl mt-2 text-diary-text/80 tracking-wider">≽^•⩊•^≼</p>
        </div>
      </div>
    </div>
  );
